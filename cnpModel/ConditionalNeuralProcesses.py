...
class ConditionalNeuralProcess(Model):
    def __init__(self, encoder_layer_widths, decoder_layer_widths, attention=False, attention_params={},
        ...
        if not attention and not convolutional:
            ...
            self._encoder = SelfAttentionEncoder(encoder_layer_widths, attention_params['num_heads'],
                                                 attention_params['num_self_attention_blocks'])
            self._decoder = AttentionDecoder(decoder_layer_widths, attention_params['embedding_layer_width'],
                                             attention_params['num_heads'])
        elif convolutional:
            self._encoder = ConvolutionalEncoder(convolutional_params["number_filters"],
                                                 convolutional_params["kernel_size_encoder"])
            self._decoder = ConvolutionalDecoder(convolutional_params["number_filters"],
                                                 convolutional_params["kernel_size_decoder"],
                                                 convolutional_params["number_residual_blocks"],
