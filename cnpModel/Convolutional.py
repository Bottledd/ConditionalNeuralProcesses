...
class ConvolutionalEncoder(Layer):
    ...
    def __init__(self, number_filters, kernel_size):
        ...
        self.conv1 = tf.keras.layers.SeparableConv2D(number_filters, kernel_size, activation=None, use_bias=False,
                                                     padding='same', depthwise_constraint=NonNeg(),
                                                     pointwise_constraint=NonNeg(), bias_constraint=NonNeg())
        # self.conv2 = tf.keras.layers.SeparableConv2D(number_filters, kernel_size, activation=None, use_bias=False,
        # padding = 'same', depthwise_constraint = NonNeg(),
        # pointwise_constraint = NonNeg(), bias_constraint = NonNeg())
...
class ConvolutionalDecoder(keras.layers.Layer):
    ...
    def __init__(self, number_filters, kernel_size, number_residual_blocks=4, convolutions_per_block=1,
                 output_channels=2):
        ...
        for i in range(number_residual_blocks):
            ...
            for j in range(convolutions_per_block):
                block_layers.append(
                    tf.keras.layers.SeparableConv2D(number_filters, kernel_size, activation='relu', padding='same'))
