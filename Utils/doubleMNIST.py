...
def process_images(image_set, context_points=100, convolutional=False, ordered=False, pre_defined=False,
                   pre_defined_values=[]):
    """
    ...
    if not (convolutional):
        ...
        if not (pre_defined):
            for i in range(batch_size):
                if not (ordered):
                    context_indices[i, :] = np.random.choice(np.arange(pixel_width ** 2, dtype=np.int32),
                                                             size=context_points, replace=False)
                else:
                    context_indices[i, :] = np.arange(context_points, dtype=np.int32)
        else:
            context_indices = np.round(pre_defined_values[:, :, 0] * (pixel_width - 1)) * pixel_width + np.round(
                pre_defined_values[:, :, 1] * (pixel_width - 1))
        ...
        # create right shape for image
        image_set = image_set.reshape((batch_size, pixel_width, pixel_width, 1))
        # create the mask
...
def format_context_points_image(inputs, convolution=False):
    if not convolution:
        x_context, y_context = inputs[0], inputs[1]

        x_context = (x_context * 63).astype(np.int32)
        y_context = (y_context * 255).astype(np.int32)

        image = np.zeros((64, 64, 3), dtype=np.int32)
        image[:, :, 2] = 255
        ...
        mask, mask_times_image = inputs[0].reshape(64, 64,1), inputs[1].reshape(64,64,1)

        mask_times_image = (mask_times_image * 255).astype(np.int32)

        rgb_image = np.zeros((64, 64, 3))
        for x1 in range(64):
            for x2 in range(64):
                if mask[x1, x2] != 0:
                    rgb_image[x1, x2] = [mask_times_image[x1, x2], mask_times_image[x1, x2], mask_times_image[x1, x2]]
                else:
                    rgb_image[x1, x2] = [0, 0, 255]
...
if __name__ == '__main__':
    ...
    #test_pic = process_images(test_set[0].reshape(1,64,64))
